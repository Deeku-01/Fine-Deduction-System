    ...
    try:
        with get_db_cursor() as cursor:
            if current_user.user_type == 'driver':
                ...
                """, (current_user.l_no, current_user.id, 'KL01CA2555'))
            else:
            ...
            return render_template('view_challans.html', 
                                challans=challans,
                                user_type=current_user.user_type)
            
    except Exception as e:
        print(f"Error in view_challans: {str(e)}")  # Server-side logging
        flash('Error loading challans. Please try again.', 'danger')
        return render_template('view_challans.html', challans=[])
